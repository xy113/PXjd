...
class ParseVideoUrl{
	...
	public static function _qq($url){
		$data = array();
		preg_match('/vid=(.*?)$/', $url,$matches);
		$vid = $matches[1];
		$xml = file_get_contents('http://vv.video.qq.com/getinfo?otype=xml&vids='.$vid);
		$xml = simplexml_load_string($xml);
		$title = (array)$xml->vl->vi->ti;
		$data['url'] = $url;
		$data['swf'] = 'http://static.video.qq.com/TPout.swf?vid='.$vid.'&ref=www.tiefanwan.com';
	...
	public static function _sina($url){
		...
		$imgstr = file_get_contents('http://interface.video.sina.com.cn/interface/common/getVideoImage.php?vid='.$vid);
		return array(
			'url'=>$url,
	...
	public static function _parse56($url){
		...
		$json = file_get_contents('http://vxml.56.com/json/'.$matches[1].'/?src=out');
		if($json){
			$array = json_decode($json,true);
	...
	public static function _ku6($url){
		...
		if(!empty($matches[1])){
			$json = file_get_contents('http://v.ku6.com/fetchVideo4Player/'.$matches[1].'.html');
			if($json){
				$array = json_decode($json,true);
