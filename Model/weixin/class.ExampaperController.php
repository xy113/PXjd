...
class ExampaperController extends BaseController{
	...
	public function index(){
		...
		if ($this->checkFormSubmit()){
			...
			include template('exam_paper_submited');
		}else {
	...
	public function viewpaper(){
		...
		if ($record){
			...
			$spenttime = floor($spenttime/60).'分'.($spenttime%60).'秒';
			include template('exam_paper_view');
		}else {
			$this->notFound();
		}
	}
	...
	private function checkRecord(){
		...
		if ($record){
			...
			if (($timelast < 0) || $record['submited']){
				...
				L('viewpaper','查看试卷');
				//$this->showError('record_exists','/?m=weixin&c=home');
				$this->showSuccess('record_exists','',array(array('url'=>'/?m=weixin&c=exampaper&a=viewpaper&recordid='.$record['recordid'],
						'text'=>'viewpaper'
				)));
				return false;
			}else {
