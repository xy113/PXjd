...
function avatar($uid=0,$size='big',$img=0){
	if (!$uid) {
		$imgurl = C('STATICURL').'images/avatar_default.png';
	}else {
		$size = in_array($size, array('small','middle')) ? $size : 'big';
		$imgurl = '/?m=member&c=avatar&uid='.$uid.'&size='.$size;
	}
...
function getBackground(){
	$i = rand(0,15);
	return C('STATICURL').'images/fullbg/'.$i.'.jpg';
}
 ...
 *@param $decrypt bool 加减密选项，0加密，1减密
 *@return string
 */
...
function ismobile($mobile){
    return preg_match('/^1[3|4|5|7|8][0-9]\d{4,8}$/', $mobile);
}
